...
// <editor-fold defaultstate="collapsed" desc="Tambahan baru - by Arif">
    ...
    public function get_list_opharapp_by_params($params) {
        $sql = "SELECT A.NOAGENDA, IDPEL, A.JENIS_TRANSAKSI, NO_TIKET, A.NO_BA, TGL_PERMINTAAN, ID_UPLOAD, B.NAMA_FILE, TGL_CATAT FROM LOG_PROSES_OPHARAPP A
                LEFT JOIN UPLOAD_LOG B ON A.NOAGENDA = B.NOAGENDA AND A.NO_BA = B.NO_BA
                WHERE A.NOAGENDA LIKE ? 
                AND A.NO_BA LIKE ? 
                AND A.JENIS_TRANSAKSI LIKE ? 
                AND A.NO_TIKET LIKE ?
                AND TO_CHAR(TGL_CATAT, 'MM/DD/YYYY')  LIKE ?
